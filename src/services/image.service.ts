...
export async function previewImage(file: File, rect: CropRect): Promise<Blob> {
  ...
  fd.append(
    "meta",
    new Blob([JSON.stringify({ rect })], { type: "application/json" })
  );

  // responseType: 'blob' da dobijemo image/png kao Blob
  const res = await api.post("/api/image/preview", fd, {
    responseType: "blob",
  });
  return res.data;
}
 ...
 * Šalje file + meta JSON (samo rect).
 * Backend automatski koristi "me" config za logo overlay.
 * Backend vraća full-quality cropped PNG kao Blob.
 */
export async function generateImage(file: File, rect: CropRect): Promise<Blob> {
  const fd = new FormData();
  fd.append("file", file);
  fd.append(
    "meta",
  ...
  const res = await api.post("/api/image/generate", fd, {
    responseType: "blob",
  });
  return res.data;
}
